...
- Install the npm package in a clean environment with `npm install -g @nickm-swe/marrow@alpha` (the package publishes under the `alpha` dist-tag only — there is no `latest` yet) and run `marrow --help`; run `marrow ui-app enable` only when desktop registration is desired.