...
**What Marrow is measurably good at — and what it is not.** Two independent free-choice A/B campaigns (July 2026: 22 cold-start agent runs across this repo and llama.cpp, ground truth and scoring pre-registered before any run, costs measured as marginal tokens over per-arm controls) produced a consistent routing map:
...
| "Explain function X and its direct callees" | Quality tie; Marrow answers in **one call** with a complete callee table vs 2–3 targeted reads at similar token cost | Either (Marrow for fewer round trips) |
