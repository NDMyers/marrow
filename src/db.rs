...
pub fn init_db(db_path: &str) -> Result<Connection> {
    ...
    ensure_nodes_source_spans(&conn)?;
...
fn ensure_performance_indexes(conn: &Connection) -> Result<()> {
    ...
    ensure_nodes_fts(conn)?;
    backfill_nodes_fts(conn)?;
    Ok(())
}

fn ensure_nodes_source_spans(conn: &Connection) -> Result<()> {
    let columns = conn
    ...
    for (name, sql_type) in [
        ...
        if !columns.contains(name) {
            conn.execute(
                &format!("ALTER TABLE nodes ADD COLUMN {name} {sql_type}"),
                [],
            )?;
        }
    }
...
fn ensure_nodes_fts(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        "CREATE VIRTUAL TABLE IF NOT EXISTS nodes_fts USING fts5(
            symbol_name,
            content='nodes',
...
fn backfill_nodes_fts(conn: &Connection) -> Result<()> {
    const FTS_REBUILD_STAT: &str = "nodes_fts_rebuilt_v1";

    let rebuilt = conn
        .query_row(
            "SELECT value FROM stats WHERE key = ?1",
            rusqlite::params![FTS_REBUILD_STAT],
            |row| row.get::<_, i64>(0),
        )
        .optional()?
        .unwrap_or(0);
...
mod tests {
    ...
    #[test]
    fn nodes_fts_and_triggers_exist_after_init() {
        let conn = init_db(":memory:").unwrap();
        let table_count: i64 = conn
            ...
            .unwrap();
        assert_eq!(table_count, 1, "nodes_fts table should exist");

        let trigger_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM sqlite_master
                 WHERE type='trigger' AND name IN ('nodes_fts_ai', 'nodes_fts_ad', 'nodes_fts_au')",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(
            trigger_count, 3,
            "nodes_fts maintenance triggers should exist"
        );
    }

    #[test]
    fn nodes_fts_backfills_existing_nodes_once() {
        let dir = tempdir().unwrap();
        let db_path = dir.path().join("graph.db");
        {
            ...
            conn.execute(
                "INSERT INTO nodes (id, repo_id, file_path, language, symbol_name, symbol_type, raw_text)
                 VALUES ('r:src/lib.rs:legacy_helper', 'r', 'src/lib.rs', 'rs', 'legacy_helper', 'function', 'fn legacy_helper() {}')",
                [],
            )
            .unwrap();
        }
        ...
        let count: i64 = conn
            ...
            .unwrap();
        assert_eq!(count, 1, "existing nodes should be backfilled");
        let match_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM nodes_fts WHERE nodes_fts MATCH 'legacy*'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(match_count, 1, "legacy node should be searchable by MATCH");

        ensure_performance_indexes(&conn).unwrap();
        let repeated_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM nodes_fts", [], |row| row.get(0))
            .unwrap();
        assert_eq!(repeated_count, 1, "backfill should not duplicate rows");
        let repeated_match_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM nodes_fts WHERE nodes_fts MATCH 'legacy*'",
                [],
                |row| row.get(0),
            ...
            .unwrap();
        assert_eq!(
            repeated_match_count, 1,
            "repeated migration should preserve searchable index"
        );
    }

    #[test]
    fn nodes_fts_tracks_insert_update_and_delete() {
        let conn = init_db(":memory:").unwrap();
        conn.execute(
            "INSERT INTO repositories (id, root_path) VALUES ('r', '/tmp/r')",
            [],
        ...
        conn.execute(
            "INSERT INTO nodes (id, repo_id, file_path, language, symbol_name, symbol_type, raw_text)
             VALUES ('r:src/lib.rs:helper', 'r', 'src/lib.rs', 'rs', 'helper', 'function', 'fn helper() {}')",
            [],
        )
        .unwrap();
        let helper_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM nodes_fts WHERE nodes_fts MATCH 'helper'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(helper_count, 1, "insert trigger should index symbol");

        conn.execute(
            "UPDATE nodes SET symbol_name = 'replacement' WHERE id = 'r:src/lib.rs:helper'",
            [],
        )
        .unwrap();
        ...
        let replacement_count: i64 = conn
            ...
            .unwrap();
        assert_eq!(old_count, 0, "update trigger should remove stale symbol");
        assert_eq!(
            replacement_count, 1,
            "update trigger should index new symbol"
        );

        conn.execute("DELETE FROM nodes WHERE id = 'r:src/lib.rs:helper'", [])
            .unwrap();
        let deleted_count: i64 = conn
            ...
            .unwrap();
        assert_eq!(deleted_count, 0, "delete trigger should remove symbol");
    }
