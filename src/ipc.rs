...
/// Returns `true` if the dashboard HTTP server at port 8765 is accepting requests.
async fn dashboard_health_check() -> bool {
...
///
/// Also verifies the dashboard at port 8765 is reachable. If the IPC socket
/// is alive but port 8765 is not bound (e.g. a stale old-version daemon),
/// the old daemon is shut down and a fresh one is spawned.
pub async fn ensure_daemon_running() -> Result<()> {
    let client = default_client();

    if client.health_check().await.unwrap_or(false) {
        // IPC is alive — also verify the dashboard port is serving.
        if dashboard_health_check().await {
            return Ok(());
        }
        // Dashboard not bound: old-version daemon is alive on the socket but
        // doesn't serve port 8765. Shut it down so we can start a fresh one.
