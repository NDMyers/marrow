...
async fn stats_endpoint_omits_stale_system_tmp_missing_db_from_workspaces_and_dbs() {
    ...
    let stale_tmp_root = stale_tmp.path().canonicalize().unwrap();
    let real_missing_root = real_missing.canonicalize().unwrap();
    ...
    assert!(workspaces.iter().all(|row| {
        row["workspace_root"].as_str().unwrap_or("") != stale_tmp_root.to_string_lossy()
    }));
    assert!(dbs.iter().all(|row| {
        row["workspace_root"].as_str().unwrap_or("") != stale_tmp_root.to_string_lossy()
    }));
    assert!(workspaces.iter().any(|row| {
        row["workspace_root"].as_str().unwrap_or("") == real_missing_root.to_string_lossy()
            && row["status"] == "missing_db"
    }));
    assert!(dbs.iter().any(|row| {
        row["workspace_root"].as_str().unwrap_or("") == real_missing_root.to_string_lossy()
            && row["status"] == "missing_db"
    }));
}
...
async fn stats_endpoint_returns_empty_inventory_when_only_stale_temp_missing_db_rows_exist() {
    ...
    let registry = Registry::open(temp.path().join("registry.db")).unwrap();
    let stale_tmp = tempfile::Builder::new()
        .prefix(".tmp-dashboard-only-stale")
        .tempdir_in(std::env::temp_dir())
        .unwrap();
    ...
    assert!(body["dbs"].as_array().unwrap().is_empty());
}
